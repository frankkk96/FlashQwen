...
- 到 S14 为止,差距**随输入长度增大**(残差 = prefill 侧算力)。**S16**(mma.sync prefill-attention 重写)
  ...
  基本填平。
- 在真实对话数据(ShareGPT)上 FlashQwen **基本打平**(100.9%),且 TTFT 远低(饱和下 3–22 ms vs vLLM 的
  355–1166 ms),稳态 TPOT 略高。
...
- **ShareGPT 真实数据**(c=32):**100.9%**(1170 vs 1159);FQ TTFT 约低 22×,vLLM TPOT 低 13%。
...
**结论:** 在同精度 + 同功能下,FlashQwen 跨输入长度是 vLLM 的 **~95–98%**,真实对话数据上**基本打平(~100%)**,
