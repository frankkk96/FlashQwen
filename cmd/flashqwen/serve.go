package main

import (
	"flag"
	"log"
	"os"
	"os/signal"
	"syscall"

	"flashqwen/internal/server"
)

func runServe(args []string) {
	fs := flag.NewFlagSet("serve", flag.ExitOnError)
	model := fs.String("model", "", "model directory (required)")
	addr := fs.String("addr", ":8000", "HTTP listen address")
	slots := fs.Int("slots", 16, "max concurrent sequences")
	maxCtx := fs.Int("max-ctx", 4096, "KV / context length")
	maxWaiting := fs.Int("max-waiting", 0, "max requests waiting for admission before new ones are rejected as over-capacity (0 => 4*slots)")
	tokenBudget := fs.Int("token-budget", 1024, "total tokens computed per scheduler step (max_num_batched_tokens); smaller keeps decode steps from being dragged by big mixed prefill batches")
	prefillChunk := fs.Int("prefill-chunk", 512, "per-request prefill chunk cap per step (long_prefill_token_threshold)")
	gpuMemFraction := fs.Float64("gpu-mem-fraction", 0.9, "fraction of total VRAM the engine may use; the KV cache pool gets whatever is left after weights+activations")
	fs.Parse(args)
	if *model == "" {
		log.Fatal("serve: --model is required")
	}

	s, err := open(*model, *slots, *maxCtx, *maxWaiting, *tokenBudget, *prefillChunk, *gpuMemFraction)
	if err != nil {
		log.Fatalf("serve: %v", err)
	}
	// Tear the engine down on Ctrl-C / SIGTERM (deferred funcs don't run on signal exit).
	sig := make(chan os.Signal, 1)
	signal.Notify(sig, syscall.SIGINT, syscall.SIGTERM)
	go func() { <-sig; s.stop(); os.Exit(0) }()

	log.Printf("flashqwen serve on %s (model %q, max_ctx %d)", *addr, s.info.ID, s.info.MaxCtx)
	if err := server.New(s.gen, s.info.ID).Run(*addr); err != nil {
		s.stop()
		log.Fatal(err)
	}
}
