...
[`docs/optimization.md`](docs/optimization.md)。核心进展(conc=32,1024 输入/128 输出,占功能对齐 vLLM 的百分比):

| 步骤 | 改动 | 提交 | % vLLM |
|---|---|---|---|
| 基线 | bf16 统一调度(INT8 main = 第二阶段起点) | `08392df` | 13% |
| S3 | bf16 权重 + cuBLAS GEMM + FlashAttention-2 | `7650654` | 53% |
| S5 | 融合 QKV / gate-up GEMM | `c2f98a0` | 55% |
| S6 | FlashDecoding decode 注意力(按请求类型拆分) | `f0b1499` | 70% |
| S7 | WMMA tensor-core prefill 注意力 | `0dd4010` | 77% |
| S8 | prefill 注意力占用率 | `392cda5` | 81% |
| S10 | 调度器:max-batch-tokens 2048→1024 | `642cc28` | 89% |
| S12 | GQA 共享 FlashDecoding(每组 K/V 只读一次) | `240aaa1` | 93% |
| S14 | activation scratch 按需分配 + KV 池/越界修复 | `e5a99c8` | 93% |

差距随输入变短(decode 占比变大)而缩小:128 输入下达到 vLLM 的 **97.5%**,512 下 **96.2%**,1024 下
**92.8%**;残余差距在 prefill 侧 compute。报告里有完整的跨输入数据,以及被实测排除的各条杠杆(CUDA graph、
