...
聚合 decode 吞吐 **~98 → ~322 tok/s(~3.3×)**(batch 16)——次线性,而原因很有启发性。batch 16 的
一步搬运约 9 GB 权重却花了 ~50 ms ≈ 190 GB/s,远低于 4090 的 ~1 TB/s,说明批处理 decode **并非
受权重带宽限制**,而是**受批处理 INT8 GEMV 的计算/占用率限制**(每 token 的 FMA 量随 `B` 增长,不再
能藏在权重读取后面)。所以真正的杠杆是 decode 的 GEMV 本身——INT8 tensor-core(IMMA/DP4A)decode
GEMM 或更好的寄存器 tiling——而不是内存流量。(`lm_head` 已经和 argmax 融合、每步只读一次权重,但这
只省了流量,反过来印证了瓶颈是计算而非带宽。)
...
服务路径。接纳时仍是单序列 prefill(交错的 chunked prefill 留到后续阶段)。
