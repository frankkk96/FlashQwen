...
相比 baseline:**prefill 快约 14–18×**,且 decode 现在**几乎不随上下文变化**——
TPOT@1024 40.1 → 18.9 ms(长上下文约 2×),decode@16 49.7 → 56.9 tok/s。复现任一阶段:
...
**仍待优化:** 权重量化(INT8/INT4)来突破 decode 的带宽天花板——decode 现在每 token 要读完整
~16 GB 权重,所以 bf16 下 ~60 tok/s 就是上限。
