...
融合的批处理 `lm_head`-argmax 和更好的 GEMV tiling,与阶段 C 一起推进。

**阶段 B——连续批处理。** 静态批处理一次处理固定一组,要等组里**最慢**的序列结束才释放所有槽位——
短请求被长请求堵在后面(队头阻塞)。由于 decode kernel 已经能接收任意的运行集合(每序列槽位 +
`past_len`),连续批处理就是一个主机侧调度器(`src/scheduler.*`):让 `n_slots` 一直满载,一有槽位
空出就立刻接纳等待中的请求,每步把整个运行集合一起 decode。在一个 48 请求、输出长度散布在 16–128
token(16 槽位)的工作负载上,它比静态批处理**快约 1.4×**(聚合 **151 → 209 tok/s**);长度方差越大
差距越大。接纳时仍是单序列 prefill(交错的 chunked prefill 留到后续阶段)。

| 48 请求,input 128,output 16–128,16 槽位 | 墙钟 | 聚合 tok/s |
|---|---:|---:|
| 静态批处理 | 21.5 s | 151 |
| 连续批处理 | 15.6 s | **209** |
